// github pages moment, it is as unoptimized as it looks
(async () => {
    ...
    try {const response = await fetch(`https://api.github.com/repos/${owner}/${repo}/contents/${currentPath}`);
        ...
        if (!response.ok) {
            if (response.status === 404) {
                document.getElementById('droid-directory').innerHTML = `<p>Directory or repository not found: <code>/${currentPath}</code>. Please check the URL or repository name.</p>`;
                return;
            }
            throw new new Error(`GitHub API error: ${response.status} ${response.statusText}`);
        }
        ...
        if (currentPath !== '') {
            ...
            let previousIcon = '/meta/dir-previous.png';
            const effectiveParentForIcon = '/' + parentPathArray.join('/');
            if (currentPath === 'media' || currentPath === 'meta' || effectiveParentForIcon === '/media' || effectiveParentForIcon === '/meta' || parentPathArray.length === 0) {
                previousIcon = '/meta/dir-root.png';
            }
        ...
        document.getElementById('droid-directory').innerHTML = `<p>Failed to load repository contents. Error: ${error.message}</p>`;
    }
})();